...
def load_paper_state():
    ...
    try:
        ...
        if "paper_hidden_trade_keys" in data:
            hk = data.get("paper_hidden_trade_keys")
            if hk is None:
                state.set_paper_hidden_trade_keys(None)
            elif isinstance(hk, list):
                state.set_paper_hidden_trade_keys(hk)
            else:
                state.set_paper_hidden_trade_keys([])
        ...
        if "paper2_hidden_trade_keys" in data:
            hk2 = data.get("paper2_hidden_trade_keys")
            if hk2 is None:
                state.set_paper2_hidden_trade_keys(None)
            elif isinstance(hk2, list):
                state.set_paper2_hidden_trade_keys(hk2)
            else:
                state.set_paper2_hidden_trade_keys([])
